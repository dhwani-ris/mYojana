...
class BeneficaryScheme:
		...
		for scheme in schemes:
			...
			if doc.rules and len(doc.rules):
				...
				for group in filters:
					sql_query = " AND ".join(
						[f"{condition[0]} {condition[1]} {condition[2]}" for condition in filters[group]]
					)
					...
					if check:
						matching_counter += 1
					rule_list.append({"message": f"[{group if group else ''}]:({sql_query})", "check": check})
	...
	def validate(beneficiary, condition):
		list = frappe.get_list(
			"Beneficiary Profiling", filters=[condition, ["name", "=", beneficiary]], page_length=1
		)
	...
	def validate_conditions(beneficiary, key, conditions):
		...
		for condition in conditions:
			...
			if is_matched:
				obj["matched"] += 1
			obj["rules"].append(
				{"message": f"{condition[0]} {condition[1]} {condition[2]}", "matched": is_matched}
			)
	...
	def get_schemes(beneficiary=None):
		...
		for scheme in schemes:
			...
			if doc.rules and len(doc.rules):
				...
				percentage_sorted_list = sorted(
					denominator_sorted_list, key=lambda x: x["percentage"], reverse=True
				)

				scheme["groups"] = percentage_sorted_list
				scheme["rules"] = (
					percentage_sorted_list[0]["rules"] if len(percentage_sorted_list) > 0 else []
				)
				scheme["total_rules"] = (
					percentage_sorted_list[0]["total"] if len(percentage_sorted_list) > 0 else 0
				)
				scheme["matching_rules"] = (
					percentage_sorted_list[0]["matched"] if len(percentage_sorted_list) > 0 else 0
				)
				scheme["matching_rules_per"] = 0
		...
		res_schemes = sorted(
			res_schemes_denominator_sort, key=lambda x: x.get("matching_rules_per", 0), reverse=True
		)
		return res_schemes
