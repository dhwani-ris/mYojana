	...
	def handle_contactnumber_change(doc, family_doc):
		...
		if _doc_before_save:
			...
			if doc.get("contact_number") != _doc_before_save.get("contact_number"):
				...
				families = frappe.db.get_list(
					doctype="Primary Member",
					filters={"name": _doc_before_save.contact_number},
					fields=["name"],
				)
				...
				if family:
					...
					frappe.db.delete("Primary Member", {"name": _doc_before_save.select_primary_member})
					# print("_doc_before_save[IF->IF->IF] del_doc",del_doc)

	def delete_family(beneficiary):
		delate_family = frappe.db.delete("Primary Member", {"name": beneficiary.contact_number})
