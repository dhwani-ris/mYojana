...
def generate_random_bulk_data():
	...
	Village = frappe.get_list("Village", filters={"state": "S07"}, pluck="name")
	date_of_birth = random_date(datetime.datetime(1950, 1, 1), datetime.datetime(2010, 1, 1)).strftime(
		"%Y-%m-%d"
	)
	completed_age, completed_age_month = calculate_age(date_of_birth)
	random_oc = random.choice(Occupation)
	random_occ = frappe.get_value("Occupation", random_oc, "occupational_category")
	random_sub_center = random.choice(Sub_Centre)
	random_center = frappe.get_value("Sub Centre", random_sub_center, "centre")
	...
	name_of_department = frappe.get_value("Scheme", scheme, "name_of_department")
	random_gender = random.choice(["Male", "Female", "Transgender", "Others"])
	data = {
		"date_of_visit": random_date(datetime.datetime(2020, 1, 1), datetime.datetime(2024, 1, 1)).strftime(
			"%Y-%m-%d"
		),
		...
		**(
			{"name_of_the_camp": random.choice(frappe.get_list("Camp", pluck="name"))}
			if soi == "Camp"
			else {}
		),
		...
		data.setdefault("scheme_table", []).append(
			{
				...
				**(
					{
						"date_of_completion": datetime.datetime.now().strftime("%Y-%m-%d")
						if application_submitted == "Completed"
						else ""
					}
				),
		...
		data.setdefault("follow_up_table", []).append(
			{
				...
				"follow_up_mode": random.choice(
					["Home visit", "Phone call", "Centre visit", "In-person visit"]
				),
				"follow_up_status": "Completed"
				if application_submitted == "Completed"
				else "Document submitted",
				"follow_up_with": random.choice(
