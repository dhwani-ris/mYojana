...
def create_user_permissions(doc=None):
	...
	if allow == "State":
		...
		new_state_doc = {"doctype": doctype, "allow": "State", "for_value": state_value, "user": user}
		...
		new_state_doc = {"doctype": doctype, "allow": "State", "for_value": state_value, "user": user}
		new_dist_doc = {"doctype": doctype, "allow": "District", "for_value": dist_value, "user": user}
		state_exist = frappe.db.exists(new_state_doc)
		...
		state_value, dist_value, block_val = frappe.db.get_value(
			"Village", for_value, ["state", "district", "block"]
		)
		new_state_doc = {"doctype": doctype, "allow": "State", "for_value": state_value, "user": user}
		new_dist_doc = {"doctype": doctype, "allow": "District", "for_value": dist_value, "user": user}
		new_block_doc = {"doctype": doctype, "allow": "Block", "for_value": block_val, "user": user}
		state_exist = frappe.db.exists(new_state_doc)
		dist_exist = frappe.db.exists(new_dist_doc)
		...
		new_state_doc = {"doctype": doctype, "allow": "State", "for_value": state_value, "user": user}
		new_centre_doc = {"doctype": doctype, "allow": "Centre", "for_value": centre_value, "user": user}
		state_exist = frappe.db.exists(new_state_doc)
