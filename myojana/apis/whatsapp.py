...
@frappe.whitelist()
def send_id(doc):
	template_name, auth_key, integrated_number = frappe.db.get_value(
		"mYojana Settings", None, ["id_card_template", "auth_key", "integrated_number"]
	)
	...
	payload = json.dumps(
		{
			...
			"payload": {
				...
				"template": {
					...
					"to_and_components": [
						{
							"to": [f"91{format_mobile_number(doc.contact_number)}"],
							"components": {
								"header_1": {
									"type": "image",
									"value": f"{frappe.local.request.scheme}://{frappe.local.request.host}{file.file_url}",
								},
								"body_1": {"type": "text", "value": f"{doc.name_of_the_beneficiary}"},
							},
						}
					],
				},
