...
def get_beneficiary_scheme_query(scheme_doc, start=0, page_limit=10, is_limit=False, filters=None):
	if filters is None:
		filters = []
	availed_sql = ""
...
def get_total_beneficiary_count_query(scheme_doc, start=0, page_limit=10, filters=None):
	if filters is None:
		filters = []
	availed_sql = ""
	if scheme_doc.get("how_many_times_can_this_scheme_be_availed") == "Once":
...
@frappe.whitelist(allow_guest=True)
def eligible_beneficiaries(scheme=None, columns=None, filters=None, start=0, page_imit=10, is_limit=False):
	...
	res = {"data": [], "count": {"total": 0, "total_family": 0, "block_count": 0, "settlement_count": 0}}
	if not scheme_doc:
		return res
...
def most_eligible_ben():
	scheam_ben_count = []
	scheame_query = """select name  from `tabScheme` """
...
def top_schemes():
	...
	for milestone in milestones:
		schemes = frappe.get_list(
			"Scheme",
			filters={"milestone": milestone.name, "name": ["IN", scheme_list], "enabled": "1"},
			fields=["name"],
		)
