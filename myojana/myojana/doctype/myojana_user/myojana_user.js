...
const openDialog = (_cb, role_profile) => {
	var doctypes = settings?.role_doctype_mapping?.filter(
		(e) => e.doctyperef && e.role_profile == role_profile
	);
	const doctypes_arr =
		settings?.role_doctype_mapping
			?.filter((e) => e.doctyperef && e.role_profile === role_profile)
			.map((e) => e.doctyperef) || [];
	// new_doctypes = doctypes.filter((e) => e.doctyperef && e.role_profile ==role_profile);
	if (!doctypes) {
		doctypes = settings?.doctype_which_is_shown_in_user_permission
			?.split(",")
			.map((e) => e.trim())
			.filter((f) => f)
			.join("\n");
...
const render_tables = async (frm) => {
	...
	for (let i = 0; i < list?.length; i++) {
        ...
        <tr>
            ...
            <td class="text-danger"><a><i class="fa fa-trash-o delete-button" id="${
				list[i].name
