...
const set_field_list = async (frm) => {
	frappe.model.with_doctype(frm.doc.select_doctype, function () {
		...
		meta.istable == 1
			? frm.set_df_property("select_parent_doctype", "hidden", 0)
			: frm.set_df_property("select_parent_doctype", "hidden", 1);
		meta.istable == 1
			? frm.set_df_property("select_parent_doctype", "reqd", 1)
			: frm.set_df_property("select_parent_doctype", "reqd", 0);
		console.log(meta);
		let meta_field = meta.fields.filter(
			(field) =>
		...
		frm.fields_dict.table_lrtz.grid.update_docfield_property(
			"select_field",
			"options",
			result
		);
