...
class BeneficiaryProfiling(Document):
	...
	def get_family(contact_number):
		docs = frappe.db.get_list(
			doctype="Primary Member",
			filters={"name": contact_number},
			fields=[
				"name",
				"name_of_head_of_family",
				"name_of_head_of_family.name_of_the_beneficiary as name_of_the_beneficiary",
			],
		)
		if len(docs):
			return docs[0]
	...
	def validate(self):
		...
		if self.what_is_the_extent_of_your_disability == "Above 40%":
			if self.proof_of_disability == []:
				return frappe.throw(
					"""Mandatory fields required in Beneficiary <br/> <br/>  &#x2022; Profiling Proof of disability"""
				)
		...
		if self.has_anyone_from_your_family_visisted_before == "No":
			if self.get("_doc_before_save", None):  # Update
				...
				if family_doc and not (family_doc.name_of_head_of_family == self.name):
					frappe.throw(
						f"Primary member exist with name <a target='_blank' href='/app/primary-member/{family_doc.name}'><b>{family_doc.name_of_the_beneficiary}</b> [{self.contact_number}]</a>, Please Select Primary Member"
					)
					return
	...
	def on_update(self):
		...
		if self.new_occupation and self.occupational_category or self.new_occupation_category:
			BeneficiaryProfiling.other_new_occupation(
				self.new_occupation, self.occupational_category, self.new_occupation_category
			)
