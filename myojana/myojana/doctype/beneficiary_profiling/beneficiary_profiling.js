...
frappe.ui.form.on("Beneficiary Profiling", {
	...
	before_save: async function (frm) {
		...
		if (
			...
			(frm.doc.alternate_contact_number || frm.doc.contact_number)
		) {
			if (!indianPhoneNumberRegex.test(frm.doc.contact_number)) {
				frappe.throw(
					`Phone Number <b>${frm.doc.contact_number}</b> set in field contact_number is not valid.`
				);
			}
		}
		if (
			!indianPhoneNumberRegex.test(frm.doc.alternate_contact_number) &&
			frm.doc.alternate_contact_number?.length > 1
		) {
			frappe.throw(
				`Phone Number <b>${frm.doc.alternate_contact_number}</b> set in field alternate_contact_number is not valid.`
			);
		}
	},
	async refresh(frm) {
		...
		if (frm.is_new()) {
			...
			frm.add_custom_button(
				__("Add family members"),
				function () {
					frappe.route_options = {
						has_anyone_from_your_family_visisted_before: "Yes",
						select_primary_member:
							frm.doc.select_primary_member || frm.doc.contact_number,
					};
			...
			let id_card_template = await frappe.db.get_single_value(
				"mYojana Settings",
				"id_card_template"
			);
			if (id_card_template) {
		...
		if (frm.doc?.current_occupation) {
			if (frm.doc.current_occupation == "Others") {
				...
				apply_filter("occupational_category", "name", frm, [
					"=",
					doc.occupational_category,
				]);
		...
		if (
			frappe.user_roles.includes("Sub-Centre") ||
			frappe.user_roles.includes("CSC Member") ||
			frappe.user_roles.includes("MIS executive")
		) {
			if (!frappe.user_roles.includes("Administrator")) {
	...
	date_of_visit: async function (frm) {
		...
		if (frm.doc.date_of_visit && frm.doc.date_of_birth) {
			if (frm.doc.date_of_visit < frm.doc.date_of_birth) {
				await frm.set_value("date_of_visit", "");
				return frappe.throw(
					"Date of registration shall not be before the <strong>Date of Birth</strong>"
				);
			}
		}
	},
	contact_number: function (frm) {
		if (
			is_primary_member_link_through_phone_number &&
			!indianPhoneNumberRegex.test(frm.doc.contact_number) &&
	...
	state: async function (frm) {
		...
		districts_option && districts_option.length > 1
			? await frm.set_value("district", "")
			: null;
	...
	current_occupation: async function (frm) {
		...
		if (frm.doc.current_occupation == "Others") {
			...
			let doc = await get_occupation_category(frm);
			await apply_filter("occupational_category", "name", frm, [
				"=",
				doc.occupational_category,
			]);
	...
	occupational_category: async function (frm) {
		frm.doc.occupational_category != "Others"
			? await frm.set_value("new_occupation_category", "")
			: null;
	},
	date_of_birth: async function (frm) {
		...
		if (dob) {
			...
			if (frm.doc.completed_age != ageString) {
				await frm.set_value("completed_age", ageString);
			}
			if (frm.doc.completed_age_month != completedAgeMonths) {
				await frm.set_value("completed_age_month", completedAgeMonths);
			}
			// await frm.set_value({
	...
	are_you_a_person_with_disability_pwd: async function (frm) {
		frm.doc.are_you_a_person_with_disability_pwd == "No"
			? await frm.set_value({
					type_of_disability: "",
					proof_of_disability: "",
	...
	what_is_the_extent_of_your_disability: async function (frm) {
		frm.doc.what_is_the_extent_of_your_disability != "Above 40%"
			? await frm.set_value("proof_of_disability", [])
			: null;
	},
	...
	social_vulnerable_category: async function (frm) {
		frm.doc.social_vulnerable_category != "Others"
			? await frm.set_value("other_social_vulnerable_category", "")
			: null;
	},
	source_of_information: async function (frm) {
		frm.doc.source_of_information != "Others"
			? await frm.set_value({
	...
	has_anyone_from_your_family_visisted_before: async function (frm) {
		if (frm.doc.has_anyone_from_your_family_visisted_before == "Yes") {
			...
			if (frm.is_new()) {
				await truncate_multiple_fields_value(frm, [
					"current_house_type",
					"state",
					"district",
					"ward",
