				...
				return {
					filters: [
						[
							...
							"module",
							"IN",
							["Sva Report", "Rule Engine", "Master", "myojana"],
						],
						["DocType", "istable", "=", 0],
						[
							"DocType",
							"name",
							"NOT IN",
							cur_frm.doc.enable_track_changes.map(function (item) {
								return item.doc;
							}),
						],
					],
...
frappe.ui.form.on("mYojana Settings", {
	...
	async before_save(frm) {
		...
		const disable_tracking = deleted_row.map((item) => {
			return item.doc;
		});
		if (disable_tracking.length) {
			await toggle_track_changes(disable_tracking, 0);
		}
		const enable_tracking = frm.doc.enable_track_changes.map((item) => {
			return item.doc;
		});
