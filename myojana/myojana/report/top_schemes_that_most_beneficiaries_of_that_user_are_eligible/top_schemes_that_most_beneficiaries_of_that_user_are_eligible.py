...
def execute(filters=None):
	...
	columns = [
		{"fieldname": "scheme", "label": "Scheme", "fieldtype": "Data", "width": 400},
	...
	get_all_scheame = frappe.db.sql("select name  from `tabScheme`", as_dict=True)
	...
	for scheme in get_all_scheame:
		...
		scheme_ben_count_list.append(
			{"scheme": scheme.name, "count": (ben_result[0].count if len(ben_result) else 0)}
		)
