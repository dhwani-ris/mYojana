class SchemeEligibleBen {
	...
	getActionBar() {
		...
		el.innerHTML = `
                <div class="d-flex " style="gap: 10px;">
                    <input id="name_of_beneficiary" type="text" class="form-control form-control-sm" placeholder="${__(
						"Name of beneficiary"
					)}">
                    <input id="name_of_parents" type="text" class="form-control form-control-sm" placeholder="${__(
						"Primary member"
					)}">
                    <input id="phone_number" type="number" class="form-control form-control-sm" placeholder="${__(
						"Phone number"
					)}">
                    <input id="block_name" type="text" class="form-control form-control-sm" placeholder="${__(
						"Block"
					)}">
                    ...
                    ${
                        ...
                        <tr class="grid-row">
                            ...
                            <td class="col grid-static-col col-xs-3 ">
                                <a href="/app/beneficiary-profiling/${item?.name}">${
											item.name_of_the_beneficiary
										}</a>
                            </td>
                            <td style="white-space: nowrap;">${item.name_of_parents ?? ``}</td>
                            <td style="white-space: nowrap;">${item.contact_number ?? ``}</td>
                            <td style="white-space: nowrap;font-size: 12px !important;">${
								item.block_name ?? ``
							}</td>
                            <td style="white-space: nowrap;font-size: 12px !important;">${
								item.village_name ?? ``
	...
	async show_table() {
		...
		this.eligible_ben = await callAPI({
			...
			args: {
				...
				columns: [
					...
					"serial_no",
					"name_of_the_beneficiary",
					"select_primary_member.name_of_parents",
					"contact_number",
					"ward.block_name",
					"name_of_the_settlement.village_name",
				],
		...
		$(document)
			.off("keyup", "#name_of_beneficiary")
			.on("keyup", "#name_of_beneficiary", (e) => {
		...
		$(document)
			.off("keyup", "#name_of_parents")
			.on("keyup", "#name_of_parents", (e) => {
		...
		$(document)
			.off("keyup", "#block_name")
			.on("keyup", "#block_name", (e) => {
		...
		// Export
		$(document)
			.off("click", "#bulkDeleteButton")
			.on("click", "#bulkDeleteButton", async (e) => {
	...
	convertJSONToCSV(jsonData, fileName) {
		...
		if (navigator.msSaveBlob) {
			...
			if (link.download !== undefined) {
				...
				alert(
					"Your browser does not support downloading files. Please try a different browser."
				);
			}
		}
	}
