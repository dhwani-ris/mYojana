...
const get_ordered_list = async (doctype, optionsToSort) => {
	...
	if (optionsToSort) {
		...
		const exceptionList = await list?.filter(
			(item) => !optionsToSort.some((item2) => item.value === item2)
		);
...
async function render_scheme_datatable(frm) {
			...
			},
			{
				...
				format: (value, columns, ops, row) => {
					...
					return `<p title="${messages.join("\n--------------   \n")}">${row?.groups
						...
						?.length?.toString()
						?.bold()}/${row?.groups?.length?.toString()?.bold()}</p>`;
				},
			},
			//milestone
			// {
	...
	let scheme_row_list = scheme_list
		.map((scheme, i) => {
			return (
				scheme.available && {
