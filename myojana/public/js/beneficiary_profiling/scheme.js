...
const showRules = (row) => {
	let rules = (row?.rules || [])
		.map((e) => `${e.rule_field} ${e.operator} ${e.data} ${e.check}`)
		.join("\n");
	frappe.msgprint({
		title: __("rules"),
		message: rules,
...
frappe.ui.form.on("Scheme Child", {
	form_render: async function (frm, cdt, cdn) {
		...
		let schemes_op = frm.doc.scheme_table
			.filter((f) => ["Open", "Under process", "Closed", ""].includes(f.status))
			.map((e) => e.name_of_the_scheme);
		let fl_schemes_ops = scheme_list.filter(
			(f) => !schemes_op.includes(f.name) && f.available
		);
		let milestones = {};
		...
		frm.fields_dict.scheme_table.grid.open_grid_row.fields_dict.milestone_category._data =
