...
const apply_filter_on_id_document = async () => {
	...
	if (child_table) {
		try {
			child_table.get_field("which_of_the_following_id_documents_do_you_have").get_query =
				function () {
					return {
						filters: [
							[
								...
								"document",
								"NOT IN",
								cur_frm.doc.id_table_list.map(function (item) {
...
frappe.ui.form.on("ID Document Child", {
	...
	enter_id_number: async function (frm, cdt, cdn) {
		...
		if (
			!indianVoterIdRegex.test(row.enter_id_number) &&
			row.which_of_the_following_id_documents_do_you_have === "Voter ID card" &&
			(row.enter_id_number.length < 11 || row.enter_id_number.length > 9)
		) {
			row.enter_id_number = "";
		...
		if (
			!indianAadharRegex.test(row.enter_id_number) &&
			row.which_of_the_following_id_documents_do_you_have === "Aadhar card" &&
			(row.enter_id_number.length < 12 || row.enter_id_number.length > 11)
		) {
			row.enter_id_number = "";
			frm.refresh_fields("enter_id_number");
