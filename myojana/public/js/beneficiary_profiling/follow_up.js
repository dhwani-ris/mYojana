...
frappe.ui.form.on("Follow Up Child", {
	form_render: async function (frm, cdt, cdn) {
		...
		if (frm.fields_dict.follow_up_table.grid && row.name_of_the_scheme) {
			let _local_scheme_followups = frm.doc.follow_up_table
				.filter((f) => f.__islocal)
				.map((e) => e.name_of_the_scheme);
			let support_data = frm.doc.scheme_table
				.filter(
					(f) =>
						["Open", "Under process", "Closed"].includes(f.status) &&
						!_local_scheme_followups.includes(f.scheme)
				)
				.map((m) => m.name_of_the_scheme);
			frm.fields_dict.follow_up_table.grid.update_docfield_property(
				"name_of_the_scheme",
				"options",
				[...support_data, row.name_of_the_scheme]
			);
		}
	},
	async follow_up_table_add(frm, cdt, cdn) {
		...
		if (frappe.user_roles.includes("Sub-Centre")) {
			...
			frm.fields_dict.follow_up_table.grid.update_docfield_property("follow", "options", [
	...
	name_of_the_scheme: function (frm, cdt, cdn) {
		let row = frappe.get_doc(cdt, cdn);
		let supports = frm.doc.scheme_table.filter(
			(f) =>
				f.scheme == row.name_of_the_scheme &&
				["Open", "Under process", "Closed"].includes(f.status)
		);
		...
		for (support_items of frm.doc.scheme_table) {
			if (row.name_of_the_scheme == support_items.name_of_the_scheme) {
				if (
					...
					support_items.application_submitted == "No"
				) {
					frm.fields_dict.follow_up_table.grid.update_docfield_property(
						"follow_up_with",
						"options",
					...
					frm.fields_dict.follow_up_table.grid.update_docfield_property(
						"follow_up_with",
						"options",
						["Beneficiary", "Government department", "Government website", "Others"]
					);
					frm.fields_dict.follow_up_table.grid.update_docfield_property(
						"follow_up_status",
						"options",
					...
					frm.fields_dict.follow_up_table.grid.update_docfield_property(
						"follow_up_status",
						"options",
						["Interested", "Not interested", "Document submitted", "Not reachable"]
					);
				}
			}
		}
	...
	follow_up_date: function (frm, cdt, cdn) {
		...
		if (row.follow_up_date < frm.doc.date_of_visit) {
			row.follow_up_date = null;
			frappe.throw(
				__("Follow-up date should not be less than date of date of registration")
			);
		}
	},
	follow_up_with: function (frm, cdt, cdn) {
		...
		if (row.follow_up_with == "Government department" || row.follow_up_with == "Others") {
			frm.fields_dict.follow_up_table.grid.update_docfield_property(
				"follow_up_mode",
				"options",
				["Phone call", "In-person visit"]
			);
		} else {
			frm.fields_dict.follow_up_table.grid.update_docfield_property(
				"follow_up_mode",
				"options",
				["Phone call", "Home visit", "Centre visit"]
			);
		}
		let supports = frm.doc.scheme_table.filter(
			(f) => f.specific_support_type == row.support_name
		);
		...
		if (row.follow_up_with != "Beneficiary" && latestSupport.application_submitted == "Yes") {
			...
			row.follow_up_with == "Beneficiary" &&
			latestSupport.application_submitted == "Yes"
		) {
			frm.fields_dict.follow_up_table.grid.update_docfield_property(
				"follow_up_status",
				"options",
			...
			row.follow_up_with == "Beneficiary" &&
			latestSupport.application_submitted == "No"
		) {
			frm.fields_dict.follow_up_table.grid.update_docfield_property(
				"follow_up_status",
				"options",
	...
	follow_up_status: function (frm, cdt, cdn) {
		...
		if (row.follow_up_status === "Not reachable" && latestSupport.status != "Closed") {
			let followups = frm.doc.follow_up_table.filter(
				(f) =>
					f.parent_ref == row.parent_ref &&
					f.support_name == row.support_name &&
					f.follow_up_status == "Not reachable"
			);
			if (followups.length >= 2) {
				frappe.warn(
					"Do you want to close the scheme?",
