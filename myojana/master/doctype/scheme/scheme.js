...
function generateQueryString(rows) {
	...
	for (let row of rows) {
		if (row.rule_field && row.operator && row.data) {
			let val = ["IN", "NOT IN"].includes(row.operator)
				? row.data
						?.split(",")
						.map((e) => `'${e}'`)
						.join(",")
	...
	let cond = Object.keys(obj)
		.map((e) => `(${obj[e].join(" AND ")})`)
		.join(" OR ");
	document.getElementById("query").innerText = cond
		? cond
		: "Rules are not set for this scheme.";
}
var field_list = [];
...
const render_counts = async (response) => {
	document.getElementById("total")
		? (document.getElementById("total").innerText =
				"Total: Beneficiary: " + response?.count?.total + ",")
		: "";
	document.getElementById("total_family")
		? (document.getElementById("total_family").innerText =
				"Primary member: " + response?.count?.family_count + ",")
		: "";
	document.getElementById("block_count")
		? (document.getElementById("block_count").innerText =
				"Block count: " + response?.count?.block_count + ",")
		: "";
	document.getElementById("settlement_count")
		? (document.getElementById("settlement_count").innerText =
				"Settlement count: " + response?.count?.settlement_count)
		: "";
};
var page_list;
const render_table = async (frm) => {
	...
	for (let i = 1; i <= total_page; i++) {
		page_list =
			page_list + `<li class="page-item"><a class="page-link page-value">${i}</a></li>`;
	...
	addTableFilter(
		datatable,
		["name_of_the_beneficiary", "name_of_parents", "contact_number", "block_name"],
		response?.data
	);
	...
	frm.set_query("name_of_department", () => {
...
const form_events = {
	[`${child_table_field}_add`]: (frm, cdt, cdn) => {
		...
		if (row.idx <= 26) {
			...
			row.code =
				String.fromCharCode(initial_code + (row.idx - 26)) +
				String.fromCharCode(initial_code + (row.idx - 26));
		}
		get_field_list("rules", frm);
	},
