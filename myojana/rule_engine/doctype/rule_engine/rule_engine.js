	...
	for (let row of rows) {
		if (["IN", "NOT IN"].includes(row.operator)) {
			...
			expression = expression.replace(
				new RegExp(row.code, "g"),
				`${row.rule_field} ${row.operator} (${val})`
			);
		} else {
			expression = expression.replace(
				new RegExp(row.code, "g"),
				`${row.rule_field} ${row.operator} '${row.data}'`
			);
		}
	}
	...
	frappe.call({
		...
		callback: function (response) {
			...
			if (response.message) {
				...
				frm.fields_dict.field_table.grid.update_docfield_property(
					"rule_field",
					"options",
					response.message
				);
...
frappe.ui.form.on("Rule Engine", {
	...
	condition: (frm) => {
		...
		if (rows && rows?.length) {
			...
			let result = evaluateExpression(input, frm.doc.condition);
