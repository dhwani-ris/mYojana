...
class DocTypeInfo:
	def get_fields(
		...
		for field in ref_doc_meta.fields:
			...
			if (
				...
				or (degree == 1 and section == "1" and field.fieldtype in ["Section Break", "Tab Break"])
			) and (field.print_hide != 1 and field.hidden != 1):
				fields.append(
					{
						"idx": float(f"{idx}.{field.idx}") if idx else field.idx,
						"doc_type": doc_type,
		...
		for field in fields:
			if field.get("child_table") is None and field.get("link_table"):
				...
				if lnk_fieldname not in unique_link_table:
					joins.append(
						f"LEFT JOIN {tbl_name} as {lnk_fieldname} ON {lnk_fieldname}.`name` = {base_tbl}.{lnk_fieldname}"
					)
				...
				if lnk_fieldname not in unique_child_table:
					joins.append(
						f"LEFT JOIN {tbl_name} as {lnk_fieldname} ON {lnk_fieldname}.`parent` = {base_tbl}.name"
					)
				...
				if lnk_fieldname_ not in unique_child_table:
					joins.append(
						f"LEFT JOIN {tbl_name_} as {lnk_fieldname_alias_} ON {lnk_fieldname_alias_}.`parent` = {base_tbl}.name"
					)
				...
				if lnk_fieldname not in unique_child_link_table:
					joins.append(
						f"LEFT JOIN {tbl_name} as {lnk_fieldname} ON {lnk_fieldname}.`name` = {lnk_fieldname_alias_}.parent"
					)
	...
	def prepare_columns(base_tbl, fields):
		...
		for field in fields:
			...
			if len(field.get("fieldname").split(".")) == 3:
				columns.append(
					f"{field.get('fieldname').split('.')[1]}.{field.get('fieldname').split('.')[2]} as `{label}`"
				)
	...
	def prepare_condition(filters, rpt_filters):
		...
		outer_tbl_cond_str = (
			f"WHERE {'AND'.join(outer_tbl_condition)}" if len(outer_tbl_condition) > 0 else ""
		)
	...
	def get_fields_info(fields):
		obj = {"columns": ["name"], "_columns": [], "children": {}}
		for field in fields:
	...
	def create_data(rows, fields, repeat_parent=False):
		...
		table_multiselect = [
			obj.get("children").get(child_table_name)
			for child_table_name in obj.get("children")
			if obj.get("children").get(child_table_name).get("info").get("fieldtype") == "Table MultiSelect"
		]
		tables = [
			obj.get("children").get(child_table_name)
			for child_table_name in obj.get("children")
		...
		for row in rows:
			...
			for child_table in table_multiselect:
				...
				records = frappe.get_list(
					ct_info.get("options"),
					fields=ct_columns,
					filters={"parent": row.name, "parenttype": ct_info.get("parenttype")},
	...
	def write_csv_data(report_doc, fields, fields_info, filters):
		...
		while True:
			...
			for result in results:
				...
				csv_writer.writerow(
					[
						f"{result.get(field.get('fieldname'), '') if result.get(field.get('fieldname'), '') else ''}"
						for field in fields
					]
				)
			yield csv_buffer.getvalue()
	...
	def get_data(doc_type, doc_name, filters=None, skip=0, limit=20, csv_export="0", debug=False):
		# return "doc_name"
		if filters is None:
			filters = []
		...
		if report_doc.ref_doctype is not None:
			if csv_export == "1":
				response = Response(
					DocTypeInfo.write_csv_data(report_doc, fields, fields_info, filters),
				...
				count_res = frappe.get_list(
					report_doc.ref_doctype, fields=["count(name) as count"], filters=filters
				)
				if len(count_res):
	...
	def get_data_old(doc_type, doc_name, filters=None, skip=0, limit=20, csv_export="1", debug=False):
		if filters is None:
			filters = []
		report_doc = frappe.get_doc(doc_type, doc_name)
		fields = DocTypeInfo.prepare_fields(report_doc)
		# print("fields",fields,report_doc)
		with_query = DocTypeInfo.prepare_base_query(
			report_doc.ref_doctype, fields, report_doc.filters, filters
		)
